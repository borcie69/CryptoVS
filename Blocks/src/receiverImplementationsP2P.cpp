...
receiver::receiver(const char *ip,const char *packet)
{
	...
	//checking if socket is viable,NPP
	if(ConnectSocket==INVALID_SOCKET)
	{
	...
	if(iResultR>0 && recvbuf==sendbuf)
	{
		printf("Block is accepted: %s\n",recvbuf);
	}
	else if(recvbuf!=sendbuf)
	{
		printf("Block is invalid: %s\n",recvbuf);
	}
	else if(iResultR==0)
	{
